...
현재 배포 버전은 **1.0.8 (versionCode 9)**이며, 배포별 변경사항은 [RELEASE_NOTES.md](RELEASE_NOTES.md)에 기록합니다. 사용자가 버전 유지를 요청한 수정은 같은 버전에 포함하고, 실제 신규 배포가 필요할 때만 `versionName`과 `versionCode`를 함께 올립니다.
