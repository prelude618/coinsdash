...
Google 로그인 뒤 Firebase UID에 연결된 Coinsdance 아이디를 확인합니다. 신규 사용자와 아이디가 없는 기존 사용자는 메인 화면 진입 전에 필수 아이디를 한 번 설정하며, 대소문자 구분 없이 중복된 아이디는 사용할 수 없습니다. 기존 사용자는 로그아웃하거나 재가입하지 않아도 기존 UID와 데이터에 아이디만 연결됩니다. 설정된 아이디는 현황 첫 화면과 설정 메뉴에 표시됩니다.
