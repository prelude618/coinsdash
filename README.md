...
현재 배포 버전은 **1.0.8 (versionCode 9)**이며, 배포별 변경사항은 [RELEASE_NOTES.md](RELEASE_NOTES.md)에 기록합니다. 별도 요청이 없으면 모든 신규 배포에서 `versionName`과 `versionCode`를 함께 올립니다.
...
현황 화면은 `총투자금액`, `보유현금`, `총매수원가`, `총보유자산`, `총코인평가액`을 5초마다 갱신합니다. 코인 화면은 모든 등록 종목을 심볼만으로 표시하고 보유 여부, 신규 매수 대상 여부, 총매수원가, 현재평가액과 등락률을 함께 보여줍니다. 심볼 검색, 보유·매수대상·등락 상태, 최소 원가·평가액으로 필터링할 수 있으며 모든 표시 필드로 오름차순·내림차순 정렬할 수 있습니다. 거래내역도 모든 거래가 KRW 마켓이라는 전제에서 `KRW-` 접두사를 생략합니다. UTC 절대시각으로 최신순 정렬한 후 Android 기기의 현지 시간대로 표시하므로 한국 날짜와 미국 날짜가 달라도 최신 거래가 누락되지 않습니다. 매도 거래에는 서버가 체결 당시 공식 평단과 매수·매도 수수료를 반영해 영구 저장한 실현 순수익을 표시한다. 해당 기능 배포 전 거래와 외부·수동 매도는 정확한 당시 원가가 없으므로 `실수익 집계 전`으로 표시한다.
...
등록 코인 목록은 매수 대상 여부와 별개로 모든 종목에 `보유` 또는 `미보유`를 명시한다. 서버는 같은 조회 주기의 업비트 실잔고·공식 평단·현재가로 종목별 총매수원가, 현재평가액과 등락률을 계산한다. 앱에서는 불필요한 `KRW-` 접두사를 숨기며 이 다섯 필드를 각각 필터링하고 정렬할 수 있다.
